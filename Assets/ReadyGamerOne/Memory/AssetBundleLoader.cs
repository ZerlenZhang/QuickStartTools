...
namespace ReadyGamerOne.MemorySystem
{
    ...
    internal class AssetBundleLoader
    {
        ...
        private IOriginPathData originPathData;
        ...
        /// </summary>
        /// <returns></returns>
        internal IEnumerator StartBundleManager(IHotUpdatePath pather, IOriginPathData originPathData = null)
        {
            this.originPathData = originPathData;
            ...
            loadLoaderTool = new AssetBundleLoaderTool(OnAddAssetBundle, assetBundleDownLoader, pather, originPathData);
        ...
        {
            Assert.IsNotNull(originPathData);
            
            var bundleName = originPathData.KeyToName[bundleNameKey];
            
            //待依赖添加资源
            AddBundleWithDependencies(bundleName,originPathData);
        ...
        void AddBundleWithDependencies(string _bundleName,IOriginPathData _originPathData)
        {
            //如果已经加载过，就无需在加载
            if (HeldedBundles.ContainsKey(_bundleName))
            {
            ...
            if (null == dependences)
            {
                loadLoaderTool.LoadFile(_originPathData.NameToPath[_bundleName]);
                return;
            }
            ...
            {
                //添加依赖，保证已经加载这个依赖
                AddBundleWithDependencies(dependence,_originPathData);
        ...
        private class AssetBundleLoaderTool
        {
            internal AssetBundleLoaderTool(Action<AssetBundle> onLoadEveryComplete, AssetBundleDownLoader version,
                IHotUpdatePath updatePath, IOriginPathData originPathData)
            {
